print("############# Running Prototype 2 Module #############")
...
def pastFiller():
    ...
    for i in range(60):
        ...
        prepData = this2that(pd.DataFrame(cData["Consumption in Mega Units"]).iloc[-60-i:len(cData)-i,:].to_numpy().reshape(1,-1)[0], True, SScaler, MMScaler, lmbda).reshape(1,60,1)
        pData7 = model.predict(prepData)
        pData = that2this(np.array(float(pData7[0])).reshape(-1,1), lmbda, SScaler, MMScaler)[0]
...
f = open("pickles/PreP_V1/lmbda", "rb")
lmbda = pickle.load(f)
f.close()

f = open("pickles/PreP_V1/SScaler", "rb")
SScaler = pickle.load(f)
f.close()

f = open("pickles/PreP_V1/MMScaler", "rb")
...
print("Loading Prototype 2 model...")
model = load_model("NewSchoolV1/")
prepData = this2that(pd.DataFrame(cData["Consumption in Mega Units"]).iloc[-60:,:].to_numpy().reshape(1,-1)[0], True, SScaler, MMScaler, lmbda).reshape(1,60,1)
pData7 = model.predict(prepData)
pData = round(float(that2this(np.array(float(pData7[0])).reshape(-1,1), lmbda, SScaler, MMScaler)[0]),2)

# cDate = finalMU["Date (YY-MM-DD)"].to_numpy()[-1].split(".")
# cDate = f"{cDate[2]}/{cDate[1]}/20{cDate[0]}"
print(f"Prediction for tomorrow from Prototype 2 {pData} MU")
